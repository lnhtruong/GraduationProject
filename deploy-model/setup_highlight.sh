#!/bin/bash
# One-shot setup: fresh Vast.ai/VPS box -> running highlight-reel API.
# Usage:
#   bash setup_highlight.sh
#   # or, to fill secrets in the same run:
#   OPENAI_API_KEY=sk-... CLOUDINARY_NAME=... CLOUDINARY_KEY=... CLOUDINARY_SECRET=... QSTASH_TOKEN=... bash setup_highlight.sh
# Safe to re-run: re-running just reinstalls/updates and restarts the service.
set -euo pipefail

# ---- paths / service (override via env before invoking if needed) ----
APP_DIR="${APP_DIR:-/opt/app_highlight}"
VENV_DIR="${VENV_DIR:-/opt/venv_highlight}"
OUTPUT_DIR="${OUTPUT_DIR:-/opt/outputs_highlight}"
TEMP_DIR="${TEMP_DIR:-/opt/highlight_temp}"
MODEL_CACHE_DIR="${MODEL_CACHE_DIR:-/opt/models_highlight}"
SERVICE_NAME="${SERVICE_NAME:-highlight-reel-api}"
PORT="${PORT:-8010}"
PYTHON_BIN="${PYTHON_BIN:-python3}"
HEALTH_TIMEOUT_SECONDS="${HEALTH_TIMEOUT_SECONDS:-1800}"

# ---- app config / secrets (blank = fill in ${APP_DIR}/.env afterwards) ----
OPENAI_API_KEY="${OPENAI_API_KEY:-}"
OPENAI_MODEL="${OPENAI_MODEL:-gpt-5.6-luna}"
QSTASH_TOKEN="${QSTASH_TOKEN:-}"
QSTASH_URL_GROUP="${QSTASH_URL_GROUP:-ai-results}"
CLOUDINARY_NAME="${CLOUDINARY_NAME:-}"
CLOUDINARY_KEY="${CLOUDINARY_KEY:-}"
CLOUDINARY_SECRET="${CLOUDINARY_SECRET:-}"
WHISPER_MODEL="${WHISPER_MODEL:-large-v3-turbo}"
LLM_MODEL_ID="${LLM_MODEL_ID:-Qwen/Qwen2.5-1.5B-Instruct}"
EMBED_MODEL_ID="${EMBED_MODEL_ID:-sentence-transformers/paraphrase-multilingual-mpnet-base-v2}"
MAX_CONCURRENT_GPU_JOBS="${MAX_CONCURRENT_GPU_JOBS:-1}"
MAX_RENDER_WORKERS="${MAX_RENDER_WORKERS:-2}"
SUBTITLE_FONT_NAME="${SUBTITLE_FONT_NAME:-Noto Sans}"
SUBTITLE_FORCE_STYLE="${SUBTITLE_FORCE_STYLE:-FontName=Noto Sans,FontSize=20,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,BorderStyle=1,Outline=2,Shadow=0,Alignment=2,MarginV=24}"

echo "== [1/7] apt packages =="
export DEBIAN_FRONTEND=noninteractive
apt-get update -qq
apt-get install -y -qq --no-install-recommends \
  build-essential ffmpeg fontconfig fonts-noto-core \
  python3 python3-dev python3-venv supervisor curl ca-certificates > /dev/null
fc-cache -f > /dev/null

echo "== [2/7] directories =="
mkdir -p "${APP_DIR}" "${OUTPUT_DIR}" "${TEMP_DIR}" "${MODEL_CACHE_DIR}"

echo "== [3/7] python venv + packages (this is the slow step, ~5-10 min) =="
if [ ! -d "${VENV_DIR}" ]; then
  "${PYTHON_BIN}" -m venv "${VENV_DIR}"
fi
"${VENV_DIR}/bin/python" -m pip install --upgrade pip setuptools wheel -q

# Colab ships torch preinstalled; a bare VPS doesn't - CUDA 12.1 wheels, proven combo in this repo.
...
"${VENV_DIR}/bin/python" -m pip install --no-cache-dir -q \
  numpy==1.26.4 \
  fastapi "uvicorn[standard]" python-multipart nest_asyncio \
  faster-whisper ctranslate2 \
  "transformers>=4.46,<4.50" "accelerate>=1.0,<1.3" "tokenizers>=0.20" \
  "huggingface_hub>=0.26" "sentence-transformers>=3.0,<4.0" "bitsandbytes>=0.43.1" \
  cloudinary requests qstash "openai>=1.40" "fonttools>=4.50"

echo "== [4/7] writing main.py (full ported API: /transcribe, /highlight-reel(-link), /highlight-edit-link, /generate-quiz) =="
cat > "${APP_DIR}/main.py" << '___HIGHLIGHT_MAIN_PY_EOF___'
# --- IMPORTS ---
...
QSTASH_TOKEN=${QSTASH_TOKEN}
QSTASH_URL_GROUP=${QSTASH_URL_GROUP}
CLOUDINARY_NAME=${CLOUDINARY_NAME}
CLOUDINARY_KEY=${CLOUDINARY_KEY}
CLOUDINARY_SECRET=${CLOUDINARY_SECRET}
...
chmod 600 "${APP_DIR}/.env"
  ...
  echo "Keeping existing ${APP_DIR}/.env (secrets/config are preserved)."
fi

echo "== [6/7] supervisor service =="
...
environment=PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:${VENV_DIR}/bin",HIGHLIGHT_ENV_FILE="${APP_DIR}/.env",HF_HOME="${MODEL_CACHE_DIR}",TRANSFORMERS_CACHE="${MODEL_CACHE_DIR}",TORCH_HOME="${MODEL_CACHE_DIR}",PYTHONUNBUFFERED="1"
stopasgroup=true
killasgroup=true
stopsignal=TERM
stopwaitsecs=60
...
echo "Waiting for API readiness (models may download on the first run)..."
deadline=$(( $(date +%s) + HEALTH_TIMEOUT_SECONDS ))
while ! curl -fsS "http://127.0.0.1:${PORT}/" > /dev/null; do
  service_status="$(supervisorctl status "${SERVICE_NAME}" || true)"
  if [[ "${service_status}" == *"FATAL"* || "${service_status}" == *"BACKOFF"* || "${service_status}" == *"EXITED"* ]]; then
    echo "Service failed during startup: ${service_status}" >&2
    tail -100 "/var/log/${SERVICE_NAME}.err.log" >&2 || true
    exit 1
  fi
...
echo "Port:         ${PORT}  (health: curl http://localhost:${PORT}/)"
echo "Logs:         tail -f /var/log/${SERVICE_NAME}.out.log /var/log/${SERVICE_NAME}.err.log"
echo "Env file:     ${APP_DIR}/.env"
if [ -z "${OPENAI_API_KEY}" ] || [ -z "${CLOUDINARY_NAME}" ] || [ -z "${QSTASH_TOKEN}" ]; then
  echo ""
  echo "!! Secrets are still blank in ${APP_DIR}/.env - edit it (nano ${APP_DIR}/.env), then:"
