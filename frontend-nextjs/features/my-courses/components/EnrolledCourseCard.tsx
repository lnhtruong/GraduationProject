...
export function EnrolledCourseCard({ enrollment }: Props) {
  ...
  return (
    <Link href={learnLink} className="group block h-full">
      <Card className="h-full flex flex-col overflow-hidden border border-border/40 bg-card/75 backdrop-blur-xs hover:bg-card hover:border-primary/40 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 rounded-2xl p-0 gap-0">
        <div className="relative aspect-video w-full overflow-hidden bg-muted/40">
          ...
          <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity duration-300">
            <div className="flex items-center gap-2 text-white font-medium bg-primary/95 px-4 py-2 rounded-full transform translate-y-4 group-hover:translate-y-0 transition-all shadow-md shadow-primary/25">
        ...
        <CardContent className="px-4 pt-2.5 pb-5 flex-1 flex flex-col justify-between">
