...
export const followApi = createApi({
  getInstructorStats: async (instructorId: number): Promise<InstructorStats> => {
    const { data } = await apiHttpClient.get(`/api/instructors/${instructorId}/stats`);
    return {
  ...
  follow: async (instructorId: number): Promise<void> => {
    await apiHttpClient.post(`/api/instructors/${instructorId}/follow`);
  },

  unfollow: async (instructorId: number): Promise<void> => {
    await apiHttpClient.delete(`/api/instructors/${instructorId}/follow`);
  },

  getFollowingInstructors: async (): Promise<FollowingInstructor[]> => {
    const { data } = await apiHttpClient.get("/api/users/following");
    const list: unknown[] = Array.isArray(data) ? data : (data.data ?? []);
    return list.map((item) => {
      const i = item as Record<string, unknown>;
      return {
        id: i.id as number,
        firstName: (i.firstName ?? i.first_name ?? "") as string,
        lastName: (i.lastName ?? i.last_name ?? "") as string,
        avatarUrl: (i.avatarUrl ?? i.avatar_url ?? undefined) as string | undefined,
        title: (i.title ?? undefined) as string | undefined,
      };
    });
  },
});
