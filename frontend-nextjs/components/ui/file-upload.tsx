    ...
    const onFilesChange = React.useCallback(
      (originalFiles: File[]) => {
        ...
        for (const file of filesToProcess) {
          ...
          if (!rejected && propsRef.current.maxSize) {
            if (file.size > propsRef.current.maxSize) {
              rejected = true;
              rejectionMessage = `File size must be less than ${formatBytes(
          ...
          if (!rejected && propsRef.current.accept) {
            const acceptTypes = propsRef.current.accept.split(",");
...
>((props, forwardedRef) => {
  ...
  return (
    <DropzonePrimitive
      ...
      className={cn(
        "relative flex select-none flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed p-6 outline-none transition-colors cursor-pointer hover:border-primary/50 focus-visible:border-ring/50 data-[disabled]:pointer-events-none data-[disabled]:cursor-default data-[dragging]:border-primary data-[dragging]:bg-primary/5 data-[invalid]:border-destructive data-[invalid]:ring-destructive/20",
        className,
      )}
      onClick={onClick}
...
function getFileIcon(file: File) {
  ...
  if (type.startsWith("image/")) return FileIcon;
...
>((props, forwardedRef) => {
  ...
  return (
    <ItemPreviewPrimitive
      ...
      ref={forwardedRef}
      className={cn(
        "grid size-10 shrink-0 place-items-center overflow-hidden rounded-md border text-muted-foreground",
        className,
      )}
    >
    ...
    return (
      ...
      >
        ...
        <span className="sr-only">
          {itemContext.fileState.progress}% uploaded
        </span>
      </ItemProgressPrimitive>
    );
  }
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: `${itemContext.fileState.progress}%` }}
      />
      <span className="sr-only">
        {itemContext.fileState.progress}% uploaded
      </span>
