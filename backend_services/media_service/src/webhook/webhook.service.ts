...
export class WebhookService {
   ...
   * On encode finished (3) or playable (4), pull video status and persist the HLS playlist URL.
   */
  async handleBunnyStream(payload: Record<string, unknown>) {
    ...
    try {
      meta = (await this.bunnyService.getVideoStatus(videoGuid)) as Record<
        string,
        unknown
      >;
    } catch (error: unknown) {
    ...
    // HLS playlist URL is built directly (Bunny's fixed CDN path) instead of calling
    // getPlayData(), which requires CDN token auth this deployment doesn't have configured.
    const url = this.bunnyService.buildPlaylistUrl(videoGuid);
    ...
    const durationRaw = meta.length ?? meta.duration;
    const duration =
    ...
    await row.update({
      ...
      duration,
      ...(titleFromMeta ? { name: titleFromMeta } : {}),
    });
    ...
    await this.notificationService.createAndEmit({
      ...
      payload: {
        ...
        purpose:
          typeof uploadContext.purpose === 'string'
  ...
  async handleUpload(payload: CloudinaryPayload) {
    ...
    if (!userId || Number.isNaN(userId)) {
      this.logger.warn(
        'Cloudinary webhook missing valid userId in context.custom',
      );
  ...
  private resolveVideoType(
    custom: CloudinaryContextCustom | undefined,
  ): VideoType {
  ...
  private async handleCloudinaryVideo(params: {
    ...
    const publicId =
      typeof public_id === 'string' ? public_id.trim() : undefined;
    const thumbnailUrl =
      cloudName && publicId
    ...
    if (jobId) {
      ...
      if (!created) {
        ...
        this.logger.log(
          `Updated video row id=${row.id} job_id=${jobId} (video asset)`,
        );
      } else {
        this.logger.log(
  ...
  private async handleCloudinaryRawSrt(params: {
    ...
    if (!created) {
      ...
      this.logger.log(
        `Updated srt_raw_url for video id=${row.id} job_id=${jobId}`,
      );
    } else {
      this.logger.log(
  ...
  async verifyAiWebhook(
    ...
    if (!rawBody || rawBody.length === 0) {
      throw new BadRequestException(
        'Missing raw body for AI webhook verification',
      );
    }
  ...
  private buildCloudinaryVideoThumbnailUrl(
    assetUrl: string,
  ): string | undefined {
    try {
  ...
  private buildHighlightMultiJobId(
  ...
  private async handleHighlightMultiCompleted(params: {
    ...
    for (const [index, video] of videos.entries()) {
      ...
      const topicId = this.parsePositiveInt(video.topic_id) ?? index + 1;
      ...
      const srtUrl =
        this.normalizeText(video.srt_url) ?? payload.srt_url ?? null;
  ...
  async handleAIResult(payload: ai_model_result) {
    ...
    const userIdStr =
      typeof userIdRaw === 'number' ? String(userIdRaw) : userIdRaw;
    const userId =
      typeof userIdStr === 'string' && userIdStr.trim().length > 0
        ? Number(userIdStr)
        : undefined;
    ...
    if (!userId || Number.isNaN(userId)) {
      this.logger.warn(
        `AI model webhook missing valid userId. payload=${JSON.stringify(payload)}`,
      );
      return { ignored: true, reason: 'missing_user_id' };
    }
    ...
    switch (eventType) {
      ...
      case 'completed': {
        ...
        if (rawType === 'subtitle') {
          ...
          const videoIdFromPayload = this.parsePositiveInt(
            payload.video_id ?? payload.videoId,
          );

          if (
            typeof videoIdFromPayload === 'number' &&
            videoIdFromPayload > 0
          ) {
        ...
        if (rawType === 'quiz') {
          ...
          if (!lessonActivityId || !videoIdFromPayload) {
            this.logger.warn(
              'quiz completed missing lesson_activity_id/video_id',
            );
          ...
          if (payload.srt_url) {
            ...
            if (!videoRow) {
              this.logger.warn(
                `quiz webhook: video_id=${videoIdFromPayload} not found`,
              );
          ...
          const normalizedQuestions = this.normalizeQuizQuestions(
            quiz.questions,
            isInVideo,
          );
          try {
            ...
            await this.notificationService.createAndEmit({
              ...
              payload: {
                ...
                lessonActivityId:
                  createdQuiz.lessonActivityId ?? lessonActivityId,
        ...
        if (jobId) {
          ...
          if (!videoRow) {
            ...
            this.logger.log(
              `Created video row id=${videoRow.id} from AI completed job_id=${jobId}`,
            );
          } else {
  ...
  private async handleCloudinaryImage(params: {
    ...
    const resolvedName =
      original_filename?.trim() || display_name?.trim() || null;
    ...
    if (jobId) {
      ...
      if (!created) {
        ...
        this.logger.log(
          `Updated image row image_id=${row.image_id} job_id=${jobId} type=${imageType}`,
        );
      } else {
        this.logger.log(
