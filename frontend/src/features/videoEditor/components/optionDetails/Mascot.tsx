...
export default function MascotOptions({ 
  ...
  const [maxMargins, setMaxMargins] = useState<{ maxMarginX: number; maxMarginY: number } | null>(null);
  ...
  useEffect(() => {
    const calculateMargins = async () => {
      ...
      try {
        ...
        if (value.type === 'preset' && value.presetUrl) {
          ...
          mascotFile = new File([blob], `${value.presetId}.png`, { type: 'image/png' });
        } 
    ...
    calculateMargins();
  }, [value.customFile, value.presetUrl, value.scale, videoFile, value.position, value.type]);
  ...
  const canApply = value.type !== 'none' && 
        ...
        <>
          {/* Current Selection Info */}
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-semibold text-primary mb-0.5">Đã chọn Mascot</p>
              <p className="text-xs text-foreground">
                {value.type === 'preset' && (
                <>
                  <strong className="font-bold">{presetMascots.find(m => m.id === value.presetId)?.name}</strong>
                  <span className="text-muted-foreground ml-1">(Có sẵn)</span>
                </>
                )}
                {value.type === 'custom' && value.customFile && (
                <>
                  <strong className="font-bold">{value.customFile.name}</strong>
                  <span className="text-muted-foreground ml-1">(Tự tạo)</span>
                </>
                )}
              </p>
              </div>
          ...
          <div className="space-y-2 border-t pt-4">
            ...
            {/* Replace option */}
            <div
              onClick={() => onChange({ ...value, position: 'replace', margin_x: 0, margin_y: 0 })}
          ...
          {value.position !== 'replace' && (
            <>
              ...
              <div className="space-y-2 border-t pt-4">
                <div className="flex items-center justify-between">
                  <Label className="text-xs text-muted-foreground">Kích thước (Scale)</Label>
                ...
                <Slider
                  ...
                  onValueChange={([val]) => onChange({ ...value, scale: val / 10 })}
                  min={1}
                  max={20}
                  step={1}
                  disabled={isApplying}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label className="text-xs text-muted-foreground">Lề ngang (Margin X)</Label>
                  <Badge variant="secondary" className="text-xs font-mono">
                    {value.margin_x}px
                ...
                <Slider
                  ...
                  onValueChange={([margin_x]) => onChange({ ...value, margin_x })}
                  min={0}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label className="text-xs text-muted-foreground">Lề dọc (Margin Y)</Label>
                  <Badge variant="secondary" className="text-xs font-mono">
                    {value.margin_y}px
                ...
                <Slider
                  ...
                  onValueChange={([margin_y]) => onChange({ ...value, margin_y })}
                  min={0}
              ...
              <div className="bg-muted/50 rounded-lg p-3 border">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Lề được tính từ góc <span className="font-semibold text-foreground">{
                      value.position === 'top-left' ? 'trên trái' :
                      value.position === 'top-right' ? 'trên phải' :
                      value.position === 'bottom-left' ? 'dưới trái' : 'dưới phải'
                    }</span> của ảnh mascot
                  </p>
                </div>
              </div>
