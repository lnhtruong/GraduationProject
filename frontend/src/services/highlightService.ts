...
const log = {
  info: (message: string, data?: unknown) => {
    console.log(`[HighlightService] ${message}`, data || "");
  },
  error: (message: string, error?: unknown) => {
    console.error(`[HighlightService ERROR] ${message}`, error || "");
  },
  warn: (message: string, data?: unknown) => {
    console.warn(`[HighlightService WARNING] ${message}`, data || "");
  },
};
...
export async function uploadHighlightReel(
  ...
  log.info("Starting highlight reel upload", {
    fileName: file.name,
    fileSize: file.size,
    topic,
  });
  ...
  try {
    ...
    if (!jobId) {
      log.error("No job ID in response", { data });
      throw new Error("No job ID returned from server");
    }

    log.info("Upload successful", { jobId });
    return jobId;
  } catch (err) {
    log.error("uploadHighlightReel error", err);
    throw err;
  }
}
...
export async function processHighlightReel(
  ...
  try {
    ...
    log.info("Step 3: Downloading clips...");
    const clips = await downloadAndParseClips(jobId, result);
    log.info("Step 3 completed", { clipsCount: clips.length });

    log.info("=== Highlight reel workflow completed! ===");
    return clips;
  } catch (err) {
    log.error("=== Highlight reel workflow failed ===", err);
    throw err;
  }
}
...
async function downloadAndParseClips(
  ...
  try {
    ...
    if (downloadUrl) {
      log.info("Download URL found in result", { downloadUrl });
      return await downloadFromUrl(downloadUrl, jobId);
    }

    // Fallback: download by job ID
    log.info("No download URL in result, using job ID", { jobId });
    return await downloadByJobId(jobId);
  } catch (err) {
    log.error("Download and parse error", err);
    throw err;
  }
}
...
async function parseDownloadResponse(
  ...
  if (contentType.includes("application/json")) {
    ...
    if (data.clips && Array.isArray(data.clips)) {
      log.info("Clips metadata found", { count: data.clips.length });
      return jobService.parseClipsFromResult({ clips: data.clips });
    }

    // Check if response itself is an array
    if (Array.isArray(data)) {
      log.info("Response is array", { count: data.length });
      return jobService.parseClipsFromResult({ clips: data });
    }

    log.warn("Unexpected JSON format", { data });
  }
  ...
  log.info("Binary file downloaded", { name, size: blob.size, type: blob.type });
